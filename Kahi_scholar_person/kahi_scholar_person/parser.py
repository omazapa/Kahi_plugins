    ...
    if s_work["profiles"]:
        ...
        for profile_name, profile_id in s_work["profiles"].items():
                    ...
                    if len_profile_name_ >= 4:
                        ...
                        if part_mone in last_names and part_mtwo in last_names and part_one[0] in [l[0] for l in first_names]:
                            ...
                            for ln in last_names:
                                if "\\" in ln:
                                    author_found, fixed_ln = backslash_in_last_names(ln, profile_name_, first_names, last_names, len_profile_name_)
                                    if author_found:
                                        author_raw = author
                        ...
                        if part_two in last_names and part_one[0] in [l[0] for l in first_names]:
                            ...
                            for ln in last_names:
                                if "\\" in ln:
                                    author_found = backslash_in_last_names(ln, profile_name_, first_names, last_names, len_profile_name_)
                                    if author_found:
                                        author_raw = author
            ...
            if author_found and fixed_ln:
                ...
                full_name = " ".join(elm.strip().capitalize() for elm in raw_first_names) + " " + " ".join(elm.strip().capitalize() for elm in raw_last_names)
                authors_work.append({"full_name": full_name, "author": author_raw, "alias": profile_name, "scholar_id": profile_id})
                match_profiles += 1
...
def parse_scholar(reg, empty_person, verbose=0):
    ...
    if authors_work:
        for author in authors_work:
            ...
            if author["author"]:
                entry["last_names"] = author["author"].split(",")[0].capitalize().replace("-", " ")
