...
def process_one_update(scienti_reg, colav_reg, db, collection, empty_work, verbose=0):
    ...
    if scienti_author:
        ...
        if 'external_ids' in scienti_author.keys():
            ...
            author_db = db['person'].find_one(
                {'external_ids': {'$elemMatch': {'$or': author_ids}}}, {"_id": 1, "full_name": 1, "affiliations": 1, "first_names": 1, "last_names": 1})
    ...
    if "author_others" in scienti_reg.keys():
        for i, author in enumerate(scienti_reg["author_others"][1:]):
            if author["COD_RH_REF"]:
                author_db = db["person"].find_one(
                    {"external_ids.id.COD_RH": author["COD_RH_REF"]}, {"_id": 1, "full_name": 1, "affiliations": 1, "first_names": 1, "last_names": 1})
                if author_db:
                    found = False
                    for author_rec in colav_reg["authors"]:
                        if author_db["_id"] == author_rec["id"]:
                            ...
                            author_reg = db['person'].find_one(
                                # this is required to get  first_names and last_names
                                {'_id': author_rec['id']}, {"_id": 1, "full_name": 1, "first_names": 1, "last_names": 1})
